...
<!-- this is not consistent on every site; however if javascript is embedded in the HTML code.. not sure the repercussions of this.. -->
...
function CMSBClose(){
	...
	}catch(e){ window.open('cms3.0.php?mode=emailEmergency&src='+escape(window.location),'w4'); }
...
<br />
			...
			if($a=q("SELECT ID, Content, EditNotes, EditDate, Editor FROM cmsb_sections WHERE thisfolder='".$_REQUEST['thisfolder']."' AND (thispage='".$_REQUEST['thispage']."' ".(!$_REQUEST['thispage'] ? "OR thispage='index.php' OR thispage='index.htm' OR thispage='index.html'" : '').") AND section='".$_REQUEST['thissection']."' ORDER BY ID DESC", O_ARRAY)){
				...
				foreach($a as $v){
					...
					<td><a title="View source code for this history" href="cms3.0.php?disposition=history&method=static:default&ID=<?php echo $v['ID']?>" onClick="return ow(this.href,'l2_cmsb','700,700');"><img src="/images/i/edit2.gif" /></a></td>
