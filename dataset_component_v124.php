...
$datasetOutput='';
if($componentRewrite || $submode=='exportDataset'){
				...
				foreach($availableCols[$datasetGroup][$modApType][$modApHandle]['scheme'] as $handle=>$scheme){
					...
					?>><?php 
						...
						if(($scheme['sortable'] || !isset($scheme['sortable'])) && !$hideSort){ 
							?><a href="#" target="w2" title="<?php echo $scheme['title'];?>"<?php echo $aClass?' class="'.$aClass.'"':''?> onclick="return datasetSort('<?php 
							...
							//2012-02-08: this is legacy and is no longer used - I think it was only used in GLF.  It will no longer be used on the dataset creator feature I am developing
							echo $datasetReferenceFile && $datasetReferenceFileKey ? ', \''.$datasetReferenceFile.'\', \''.$datasetReferenceFileKey.'\'' : '';
			...
			while(true){
				...
				?><tr id="r_<?php echo $$datasetID?>" onclick="<?php if($useHighlighting)echo $hNormalCoding;?>" ondblclick="<?php if($useHighlighting)echo $hNormalCoding;?>open<?php echo $dataset?>();" oncontextmenu="<?php if($useHighlighting)echo $hContextMenuCoding;?>" class="normal<?php echo fmod($i,2)?' alt':''?><?php echo $datasetAdditionalClass ? $datasetAdditionalClass : ''?>" deletable="<?php echo $deletable?>" <?php echo strtolower($datasetActiveFieldLabel ? $datasetActiveFieldLabel : $datasetActiveField);?>="<?php echo $$datasetActiveField?>"<?php if($datasetCustomAttributes)echo $datasetAttributes;?>>
					...
					foreach($availableCols[$datasetGroup][$modApType][$modApHandle]['scheme'] as $handle=>$scheme){
						...
						if($scheme['method']=='function'){
							ob_start();
							eval(
							'$out='.
							($datasetFieldExpressionFunction=='parse' ? dataset_function_parse($scheme['fieldExpressionFunction']) : rtrim($scheme['fieldExpressionFunction'],';'))
							.';'
							);
						...
						}else if(!$scheme['method'] || $scheme['method']=='field'){
...
<?php
if($submode=='exportDataset'){
	//remove the output entirely - raw (csv) data is found in variable $datasetOutpu
	ob_end_clean();
}else if($componentRewrite){
