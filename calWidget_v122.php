...
version 1.22
------------
- forked this off for the Gi ocosaC are project so I could customize for them but ONLY GC is using as of 2011-03-06
...
if(!function_exists('cal_grid_display')){
	function cal_grid_display($day, $events, $options=array()){
		...
		foreach($events as $n=>$v){
			?><div class="gridEvent">y<a class="gridEventLink" title="<?php echo h($v['Description'])?>" href="<?php echo $pageHandles['calendarFocus']?>?Events_ID=<?php echo $v['ID']?>"><?php
		...
		?><div id="calSection"><?php
			...
			if(!$hideCalWidget){
				?><div id="calWidget">
					...
					<span id="nextMonth" class="<?php if($calPreventFutureNavigation > 0 && $calPreventFutureNavigation<= $year.str_pad($month,2,'0',STR_PAD_LEFT))echo 'unavailable';?>"><a href="<?php echo $pageHandles['calRequestURL']?><?php echo $pageHandles['calRequestURL']?>?mode=navMonthEventCalendar&Cal_ID=<?php echo $Cal_ID?$Cal_ID:1?>&year=<?php echo $month==12?$nextYear:$year?>&month=<?php echo $nextMonth?><?php echo $calNavQueryStringParams?>" title="View next nonth" target="w2" onclick="return calNav(this,<?php echo $month==12?$nextYear:$year?>,<?php echo $nextMonth?>);"><img src="<?php echo $calNavRightImage?>" alt="next month" align="absbottom" /></a>
					</span>
					</div>
					<?php
									...
									if($gridEventUsage=='focus'){
										...
										if(count($events) && !$calHideGridEvents){
											foreach($events as $n=>$v){
												...
												if($gridEventUsage=='focus'){
													?>x<a class="gridEventLink" title="<?php echo h($v['Description'])?>" href="<?php echo $pageHandles['calendarFocus']?>?Events_ID=<?php echo $v['ID']?>" <?php echo $gridEventOnclick?>><?php
												}
												//handle text length eventually
