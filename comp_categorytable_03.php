...
if($records=q($categorySQL, O_ARRAY)){
	...
	while(list(,$v)=each($records)){
		...
		$productSKUs=q("SELECT ID,SKU FROM finan_items i WHERE i.Active=1 AND Type='Non-inventory Part' AND Category='".addslashes($v['Category'])."'", O_COL_ASSOC);
		$SubCategoryCount=q("SELECT COUNT(DISTINCT SubCategory) FROM finan_items i WHERE i.Active=1 AND Type='Non-inventory Part' AND Category='".addslashes($v['Category'])."'", O_VALUE);
		$featured=q("SELECT COUNT(Featured) FROM finan_items i WHERE i.Active=1 AND Type='Non-inventory Part' AND Category='".addslashes($v['Category'])."' AND Featured=1", O_VALUE);
		...
		if(!$haveImage && count($productSKUs)){
			foreach($productSKUs as $y){
				if($img=get_image(strtolower($y), $$catImgBackupArray, $catImgBackupOptions)){
					...
					ob_start();
					copy($catBackupPath.'/'.$img['name'], $catPath.'/'.$haveImage);
					$err=ob_get_contents();
					ob_end_clean();
					if($err){
						$email='Hi there,
		...
		*/
		...
		?>
		<a href="<?php echo count($productSKUs)==1?$pageHandles['singlePage']: ($SubCategoryCount==1 ? $pageHandles['productsPage'] : $pageHandles['subCategoryPage'])?>?Category=<?php echo urlencode($v['Category'])?>&SubCategory=<?php echo urlencode(q("SELECT DISTINCT SubCategory FROM finan_items i WHERE i.Active=1 AND (Type='Non-inventory Part' OR Type IS NULL OR Type='') AND Category='".addslashes($v['Category'])."'", O_VALUE));?>&ID=<?php echo $singleProductID;?>" title="See products for <?php echo stripslashes($v['Category'])?>"><?php
		//show product picture
		if($haveImage){
			?><img alt="category image" <?php echo $height?'height="'.$height.'"':''?> width="<?php echo $width?>" src="<?php echo $catPath . '/'.$haveImage?>" /><?php
		}else if($defaultNAImage && file_exists(rtrim($defaultNAImagePath,'/') . '/'.$defaultNAImage)){
			?><img alt="category image unavailable" height="<?php echo $defaultNAImageHeight?>" width="<?php echo $defaultNAImageWidth?>" src="<?php echo rtrim($defaultNAImagePath,'/') . '/'.$defaultNAImage?>" /><?php
		}else{
			//do nothing
			?>&nbsp;<?php
		}
		...
		if($adminMode){
			if($allowCategoryRanking){
				...
				*/
				...
				?><div class="catRanking"><span class="ctrls">
			...
			?>
			<a title="Edit images folder" onclick="return ow(this.href,'l1_subcategory','700,700');" href="/admin/file_explorer/?folder=category&uid=category"><img src="images/i/edit2.gif" alt="edit images" width="15" height="18">&nbsp;Edit Images</a> <br>
