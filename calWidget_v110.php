<?php
$thisWidgetFile='calWidget_v110.php';
/*
cal widget 1.0 b002
-------------------
2009-08-24
...
//1.20 improvement: allows us to make "real today" any day
...
if(!$refreshComponentOnly){
	ob_start();
	?><style type="text/css">
	/* NOTE: you can override this calenar/news CSS by setting $calOverrideCalNewsCSS=true before the widget */
	#cal{
		float:right;
		width:190px;
		...
		?><div id="calSection"><?php
			...
			if(!$hideCalWidget){
				?><div id="calWidget">
					...
					?><a href="<?php echo $pageHandles['calRequestURL']?>?mode=navMonthEventCalendar&Cal_ID=<?php echo $Cal_ID?$Cal_ID:1?>&year=<?php echo $month==1?$prevYear:$year?>&month=<?php echo $prevMonth?><?php echo $calNavQueryStringParams?>" title="View previous Month" target="w2"><img src="<?php echo $calNavLeftImage?>" alt="previous month" align="absbottom" /></a> 
						<span class="monthYear"><?php echo date('F',strtotime($year.'-'.str_pad($month,2,'0',STR_PAD_LEFT).'-01'))?> <?php echo $year?></span>
						<a href="<?php echo $pageHandles['calRequestURL']?><?php echo $pageHandles['calRequestURL']?>?mode=navMonthEventCalendar&Cal_ID=<?php echo $Cal_ID?$Cal_ID:1?>&year=<?php echo $month==12?$nextYear:$year?>&month=<?php echo $nextMonth?><?php echo $calNavQueryStringParams?>" title="View next nonth" target="w2"><img src="<?php echo $calNavRightImage?>" alt="next month" align="absbottom" /></a>
					</div>
					<?php
...
if(($mode=='navMonthEventCalendar' || $mode=='fetchEventsEventCalendar') && $calSelfRefresh){
