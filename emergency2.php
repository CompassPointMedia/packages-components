...
//clear these vars each time calling this file
unset($datasourceExplained);
...
?><div id="<?php echo $datasetComponent?>" refreshparams="noparams">
	...
	?>
	<?php echo '<h'.($datasetComponentHeadingLevel ? $datasetComponentHeadingLevel : '3').' id="'.$datasetComponent.'_heading">'?><?php echo $datasetWordPlural;?> (<span id="<?php echo $datasetComponent?>_count"><?php echo $count;?></span>) <?php 
	...
	<table border="0" cellspacing="0" cellpadding="0" class="<?php echo $datasetTableClass?>" style="clear:both;">
		...
		<thead>
			<tr>
				...
				foreach($availableCols[$datasetGroup][$modApType][$modApHandle]['scheme'] as $handle=>$scheme){
					...
					?><th nowrap="nowrap" <?php echo $scheme['sortable'] || !isset($scheme['sortable']) ? 'sortable="1"' : ''?> class="<?php echo $sort==$handle ? 'sorted'.strtolower($asc):''?><?php echo $cols==$visibleColCount?' last':''?>"><?php 
						...
						if(($scheme['sortable'] || !isset($scheme['sortable'])) && !$hideSort){ 
							?><a href="#" target="w2" title="<?php echo $scheme['title'];?>" onClick="return datasetSort('<?php echo $datasetComponent?>','<?php echo $handle?>',event);"><?php
						}
						//output header
						if($submode=='exportDataset' && $datasetExportMethod>1){
							...
							if($n=$scheme['exportKey'] && $datasetExportMethod>2){
								if(is_array($n)){
									...
									(strlen($availableCols[$datasetGroup][$modApType][$modApHandle]['scheme'][$n]['header']) ? $availableCols[$datasetGroup][$modApType][$modApHandle]['scheme'][$n]['header'] : preg_replace('/([a-z])([A-Z])/','$1 $2',$n)).
				...
				if($submode=='exportDataset' && $datasetExportMethod>1){
					$datasetOutput.=rtrim($datasetOutput,$datasetExportSeparator);
				}
				?>
			</tr>
		</thead>
			...
			while(true){
				...
				?><tr id="r_<?php echo $$Dataset_ID?>" onClick="<?php if($useHighlighting)echo $hNormalCoding;?>" onDblClick="<?php if($useHighlighting)echo $hNormalCoding;?>open<?php echo $dataset?>();" oncontextmenu="<?php if($useHighlighting)echo $hContextMenuCoding;?>" class="normal<?php echo fmod($i,2)?' alt':''?><?php echo $datasetAdditionalClass ? $datasetAdditionalClass : ''?>" deletable="<?php echo $deletable?>" <?php echo strtolower($datasetActiveField);?>="<?php echo $$datasetActiveField?>"<?php if($datasetCustomAttributes)echo $datasetAttributes;?>>
					...
					if(!$hideObjectInactiveControl){ 
						?>
						<td id="r_<?php echo $$Dataset_ID?>_active" title="<?php echo $$datasetActiveField ? $datasetActiveInactivateTitle : $datasetActiveActivateTitle?>" onClick="<?php eval('echo "'.$datasetActiveControl.'";');?>" class="activetoggle"><?php
					...
					if(!$hideEditControls){
						if($datasetSetIdx){
							?><td>
							<img title="Click to move this row up or down; hold down the Ctrl key to move to the top or bottom" id="idx_<?php echo $$Dataset_ID?>" src="<?php echo $datasetSetIdxImgPath ? $datasetSetIdxImgPath : 'file:///C|/Users/Samuel Fullman/Desktop/Hosted Accounts/images/i/arrows/spinner-orange.png'?>" height="<?php echo $datasetSetIdxImgHeight ? $datasetSetIdxImgHeight : 19?>" onClick="indexRow(this,event);" />
							</td><?php
						}
					...
					foreach($availableCols[$datasetGroup][$modApType][$modApHandle]['scheme'] as $handle=>$scheme){
						...
						//-------- 2010-08-01 kernel logic moved to group_dataset_functions --------
						$out=dataset_kernel($scheme, $record);
						...
						if($submode=='exportDataset' && $datasetExportMethod>1){
							...
							?><td <?php if($format=='float')echo 'align="right"';?><?php if($scheme['width'])echo ' width="'.$scheme['width'].'"';?><?php echo $scheme['nowrap'] || $mergeAttribs['nowrap']?' nowrap':''?> class="<?php echo $sort==$handle ? 'sorted':''?><?php echo $handle==$lastHandle?' last':'' ?><?php if($mergeAttribs['class'])echo ' '.$mergeAttribs['class']?>"<?php if($colAttribs)foreach($colAttribs as $n=>$v)echo ' '.$n.'="'.$v.'"';?>><?php
							echo strlen($out) ? $out : '&nbsp;';
							?></td><?php
						}
					}
