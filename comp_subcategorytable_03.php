...
WHERE i.Active=1 AND (i.Type='Non-inventory Part' OR i.Type='') AND 
i.Category='".$Category."'
...
if($records=q($subcategorySQL, O_ARRAY)){
	...
	while(list(,$v)=each($records)){
		...
		$productSKUs=q("SELECT ID, SKU FROM finan_items i WHERE i.Active=1 AND Type='Non-inventory Part' AND Category='$Category' AND SubCategory='".$v['ActualSubCategory']."'", O_COL_ASSOC);
		$featured=q("SELECT COUNT(Featured) FROM finan_items i WHERE i.Active=1 AND Type='Non-inventory Part' AND Category='$Category' AND SubCategory='".$v['ActualSubCategory']."' AND Featured=1", O_VALUE);
		...
		if(!$haveImage && count($productSKUs)){
			foreach($productSKUs as $y){
				if($img=get_image(strtolower($y), $$subcatImgBackupArray, $subcatImgBackupOptions)){
					...
					ob_start();
					copy($subcatBackupPath.'/'.$img['name'], $subcatPath.'/'.$haveImage);
					$err=ob_get_contents();
					ob_end_clean();
					if($err){
						$email='Hi there,
		...
		if($adminMode){
			...
			?>
			<a title="Edit images folder" onclick="return ow(this.href,'l1_subcategory','700,700');" href="/admin/file_explorer/?folder=subcategory&uid=subcategory"><img src="/images/i/edit2.gif" alt="edit images" width="15" height="18">&nbsp;Edit Images</a> <br>
			<?php if(false){ ?>
