...
version 1.40 - 2010-06-04
2011-12-11
* if this gives problems enclosed in a function look for globals - changed document_root to globals.document_root for example

* from Penn I developed "config instance 1" which is the first really clean instance.
	ssSuppressComponentDiv default=true
* browser var MUST be present for slideshow to work
* just time for a change so I incremented the minor version #
* fixed Parker's mistakes on ssRemoteSource - good idea, poor execution
version 1.34 - 2011-06-04
* no major differences from 1.32 noted (1.33 skipped)
...
2011-05-12
---------
* [this is a note by parker; not very accurate it seems] got rid of references to v['path'] that would refer to the server root, 
for instance get_file_assets('/'); refers to the root of the server $ssFolder now handles those references
2009-10-05
----------
...
if(!isset($ssFolder))$ssFolder=$_SERVER['DOCUMENT_ROOT'].'/images/slides/default';
...
//box mode
if(!isset($boxMode))$boxMode=1; //1=expand box to fit, 2=contract images to fit box (images will not be overstretched), 3=make box overflow:hidden;
...
if(!isset($ssSuppressComponentDiv))$ssSuppressComponentDiv=true; //added 2011-06-05 - ssComponent div not really needed
...
/* Added By Parker On 7-9-10, available variable: */
//$ssRemoteSource='http://tp.usamls.net/getMLPhoto.asp?';
...
if(count($ssPictures)){
	?>
	<script language="javascript" src="<?php echo $slideshowGlobalJSURL ? $slideshowGlobalJSURL : 'http://www.relatebase-rfm.com/Library/js/global_slideshow_v'.($ssSlideshowJS ? $ssSlideshowJS : '131').'.js' ;?>" type="text/javascript"></script>
	...
	if($ssPictures){
		...
		foreach($ssPictures as $n=>$v){
			ob_start();
			$a=getimagesize($ssRemoteSource ? $ssRemoteSource.'/'.$v['name'] : $_SERVER['DOCUMENT_ROOT'].'/'.($v['path']?$v['path']:$ssFolder).'/'.$v['name']);
			$err=ob_get_contents();
	...
	<?php if(!$ssSuppressComponentDiv){ ?>#ssComponent{position:relative;	}<?php } ?>
	#ly2{	z-index:2; 				}
	#ly1{	z-index:1; 				}
	...
	<?php if(!$ssSuppressComponentDiv){ ?>#ssComponent{
	...
	<div id="ssWrap">
		...
		if($ssUseSlideshow){
			ob_start();
			?><?php if(!$ssSuppressComponentDiv){ ?><div id="ssComponent"><?php } ?>
				<?php ob_start();?>
				<span id="topTextWait">
			...
			<?php if(!$ssSuppressComponentDiv){ ?></div><?php } ?><?php
		...
		if($ssIntegrateText){
			...
			foreach($ssPictures as $v){
				...
				?><div id="description_<?php echo $i?>" filename="<?php echo $v['name']?>" articleid="<?php echo $v['ID']?>" <?php
				/* ---------------------------
				//handle href
				if($ssLinkMethod==1){
		...
		loadImagesStats(idx);
		<?php if($ssSlideArticleLink){ ?>
		function slideArticleLink(o){
			var Articles_ID=g('description_'+idx).getAttribute('articleid');
			window.location=o.href+Articles_ID;
			return false;
		}
		<?php } ?>
		</script>
	...
	if($ssShowGallery){
		...
		if($ssGalleryCreateThumbs && !$ssRemoteSource){
			if(is_dir($ssGalleryFolder)){
				...
				if(!$ssRemoteSource && !mkdir($ssGalleryFolder)){
					...
					foreach($ssPictures as $v){
						...
						$boundingBoxWidth=$ssGalleryWidth;
						$boundingBoxHeight=$ssGalleryHeight;
						if($a=getimagesize(($ssRemoteSource ? $ssRemoteSource : ($v['path']?$v['path']:$ssFolder)).'/'.$v['name'])){
							$imagewidth=$a[0];
							$imageheight=$a[1];
							if($boundingBoxWidth < $imagewidth || $boundingBoxHeight < $imageheight){
								//box the image as a copy, leaving the uploaded temp file as-is. NOTE, later in dev we'll move this larger file to an "originals" folder OR a master folder with some settings so that we can consider this resize a step, and then revert back to original
		...
		if(($ssGalleryThumbs=get_file_assets($ssGalleryFolder,'normal')) && !$ssRemoteSource){
			ob_start();
			?>
			...
			<div id="ssGallery">
				<?php if($ssGalleryTitle){ ?><h2><?php echo $ssGalleryTitle;?></h2><?php } ?>
				<?php ob_start();?>
			...
			?><div class="cb"> </div></div></div><script language="javascript" type="text/javascript">
