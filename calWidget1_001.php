<?php
$thisWidgetFile='calWidget1_001.php';
/*
...
if(!$overrideCalNewsCSS && !$refreshComponentOnly){
	?><style type="text/css">
...
?><div id="cal"><?php
	...
	if(!$hideNewsSection){ 
		ob_start();
		?><div id="newsSection">
			<div id="newsHeader"><?php echo $newsHeaderText?></div>
	...
	if(!$hideCalSection){
		...
		?><div id="calSection"><?php
			...
			?><div id="calHeader"><?php echo $calHeaderText?></div><?php
			...
			if(!$hideCalWidget){
				?><div id="calWidget">
					...
					<div id="calNav"><?php
					?>
						<a href="#" onClick="window.open('<?php echo $pageHandles['calRequestURL']?>?mode=navMonthEventCalendar&Cal_ID=<?php echo $Cal_ID?$Cal_ID:1?>&year=<?php echo $month==1?$prevYear:$year?>&month=<?php echo $prevMonth?><?php echo $calNavQueryStringParams?>','w2');return false;" title="View previous Month"><img src="/images/i/left_blue_triangle.jpg" alt="previous month" width="8" height="9" border="0" /></a> 
						<span style="padding:0px 8px;"><?php echo date('F',strtotime($year.'-'.str_pad($month,2,'0',STR_PAD_LEFT).'-01'))?> <?php echo $year?></span>
						<a href="#" onClick="window.open('<?php echo $pageHandles['calRequestURL']?><?php echo $pageHandles['calRequestURL']?>?mode=navMonthEventCalendar&Cal_ID=<?php echo $Cal_ID?$Cal_ID:1?>&year=<?php echo $month==12?$nextYear:$year?>&month=<?php echo $nextMonth?><?php echo $calNavQueryStringParams?>','w2');return false;" title="View next nonth"><img src="/images/i/right_blue_triangle.jpg" alt="next month" width="8" height="9" border="0" /></a>
					</div>
					<?php
					echo $calCalNav=get_contents();
					ob_start();
					?>
					<div id="calGrid"><table border="1" class="cal10" cellpadding="2" cellspacing="0"><?php
					$i=0;
					$j=0;
					for($i=1; $i<=$cells; $i++){
						if(!(($i-1)%7)){
						...
						if(is_null($thisDay))$class.=' nonday';
			...
			if(!$hideCalEventListing){
				...
				}else if(($thispage=='index_01_exe.php'?exit('test'):true) && $past && $future && $calGetRange && $events=q("SELECT * FROM 
					cal_events WHERE $calEventWhereClause
				...
				}else if($thispage!==trim($pageHandles['calList'],'/') && $month && 
