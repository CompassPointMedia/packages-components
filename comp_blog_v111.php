...
if($mode=='submitBlog'){
	if($key){
		...
		if(!$blogLoggedIn && $blogCommentRequiresLogin){
			...
			require('cgi/usemod_settings_messages.php');
			...
			?>window.parent.location='/cgi/login.php?messageCode[loginFormHeader]=blogsigninfirst&src=<?php echo urlencode($blogFocusPage.'?Articles_ID='.$ID.'&key='.$key.'&mode=submitBlog');?>';
			</script><?php
	...
	if(!$abortEntry){
		...
		Objects_ID='$Articles_ID',
		ObjectName='cms1_articles',
		".(isset($Posts_ID) ? "Posts_ID='$Posts_ID', " : '')."
		PostDate=NOW(),
