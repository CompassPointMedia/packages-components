<?php
$componentVersions['cal']=1.30;

/*
version 1.30
------------
- 2013-08-24: forked this off to start use in a component 
* still want to keep as atomic as possible
* $calPreventPastNavigation is no longer boolean but a date e.g. 2013-07-01; calendar will navigate to no earlier than July of 2013; similar for $calPreventFutureNavigation
...
if(!isset($calOverrideDefaultCSS))$calOverrideDefaultCSS=false;
...
if(!function_exists('cal_grid_display')){
	function cal_grid_display($day, $events, $options=array()){
		...
		if($calAccessToken){
			?><a href="/console/events.php?Cal_ID=<?php echo $Cal_ID;?>&StartDate=<?php echo $year.'-'.$month.'-'.str_pad($day,2,'0',STR_PAD_LEFT);?>" title="Add a new event for this day" onclick="return ow(this.href,'l1_events','700,700',true);"><img src="/images/i/plusminus-plus.gif" width="11" height="11" alt="new event" /></a> <?php
		}
		echo $day?></div>
...
if(!$refreshComponentOnly){
	ob_start();
	?><style type="text/css">
	/* NOTE: you can override this calendar/news CSS by setting $calOverrideDefaultCSS=true before the widget */
	#cal{
		}
	#calHeader{
		display:none;
	...
	$calDefaultCSS=get_contents();
	if(!$calOverrideDefaultCSS)echo $calDefaultCSS;
		...
		?><div id="calSection"><?php
			...
			if(!$hideCalWidget){
				?><div id="calWidget">
					<?php
					ob_start();
					$preventPastNavigation=(substr(preg_replace('/[^0-9]+/','',$calPreventPastNavigation),0,6) >= $year.str_pad($month,2,'0',STR_PAD_LEFT));
					$preventFutureNavigation=($calPreventFutureNavigation>0 && substr(preg_replace('/[^0-9]+/','',$calPreventFutureNavigation),0,6)<= $year.str_pad($month,2,'0',STR_PAD_LEFT));
					?>
					<div id="calNav" class="cnclr">
					<span id="prevMonth" class="<?php if($preventPastNavigation)echo 'unavailable';?>"><a href="<?php
					if($preventPastNavigation){
						echo '#';
					}else if($calNavURL){
						//2013-08-24: must contain all parameters besides year and month
						echo $calNavURL.'&Cal_ID='.$Cal_ID.'&year='.($month==1?$prevYear:$year).'&month='.$prevMonth;
					}else{
						echo $pageHandles['calRequestURL'];?>?mode=navMonthEventCalendar&Cal_ID=<?php echo $Cal_ID;?>&year=<?php echo $month==1?$prevYear:$year?>&month=<?php echo $prevMonth?><?php echo $calNavQueryStringParams;	
					}
					?>" title="<?php echo $preventPastNavigation?'Cannot go back any further':'View previous month';?>" target="w2" onclick="return calNav(this,<?php echo $month==1?$prevYear:$year?>,<?php echo $prevMonth?>);"><img src="<?php echo $calNavLeftImage?>" alt="previous month" align="absbottom" /><?php
					if($n=$calPrevMonthWord){
						echo ' '.$n;
					}
					?></a>
					</span>
					...
					<span id="nextMonth" class="<?php if($preventFutureNavigation)echo 'unavailable';?>"><a href="<?php 
					if($preventFutureNavigation){
						echo '#';
					}else if($calNavURL){
						//2013-08-24: must contain all parameters besides year and month
						echo $calNavURL.'&Cal_ID='.$Cal_ID.'&year='.($month==12?$nextYear:$year).'&month='.$nextMonth;
					}else{
						echo $pageHandles['calRequestURL']?>?mode=navMonthEventCalendar&Cal_ID=<?php echo $Cal_ID;?>&year=<?php echo $month==12?$nextYear:$year?>&month=<?php echo $nextMonth?><?php echo $calNavQueryStringParams;
					}
					?>" title="<?php echo $preventFutureNavigation?'Cannot go forward any further':'View next month';?>" target="w2" onclick="return calNav(this,<?php echo $month==12?$nextYear:$year?>,<?php echo $nextMonth?>);"><?php
					if($n=$calNextMonthWord){
						echo ' ' .$n;
					}
					?><img src="<?php echo $calNavRightImage?>" alt="next month" align="absbottom" /></a>
					</span>
					</div>
					<?php
					...
					for($i=1; $i<=$cells; $i++){
						...
						if($calGridDisplayFunction){
							...
							if(is_null($thisDay)){
								...
								//------------------ gridDay and usage ------------------
								if($calAccessToken){
									?><span class="linkAnchor"><a href="/console/events.php?StartDate=<?php echo $year.'-'.$month.'-'.str_pad($thisDay,2,'0',STR_PAD_LEFT);?>" title="Add a new event for this day" onclick="return ow(this.href,'l1_events','700,700',true);" class="_editLink_1"><img src="/images/i/plusminus-plus.gif" width="11" height="11" alt="new event" /></a></span><?php
								}
			...
			if(!$calHideEventListing){
				...
				if($calAccessToken){
					?><a title="Edit this event" href="/console/events.php?Cal_ID=<?php echo $Cal_ID;?>&cbFunction=refreshList" onclick="return ow(this.href,'l1_events','700,700',true);"><img src="/images/i/plusminus-plus.gif" width="11" height="11" alt="edit" />&nbsp;
					New event</a><?php
				}
				?></div><?php
