...
/*
2013-01-16:
	* $datasetArrayType ? $datasetArrayType : O_ARRAY (vs O_ARRAY_ASSOC)
2012-12-21
	* sort and add next batch type controls both seem to be working and storing in bais_settings
	* $datasetUserName is now set by function sun() - must be working right! but finally abstracted
2012-12-18: version 2.00alpha
	* see comments in dataset_component_v200a.php
	
*/
if(!$datasetCnx)$datasetCnx=$qx['defCnxMethod'];
...
if(!$dataset){
	mail($developerEmail, 'Error file '.__FILE__.', line '.__LINE__,get_globals($err='Variable $dataset has not been declared'),$fromHdrBugs);
	error_alert($err);
}
if($componentFile && ($componentKey!==$datasetReferenceFileKey || !$componentKey)){
	mail($developerEmail, 'Error file '.__FILE__.', line '.__LINE__,get_globals($err='New method of sorting component appears to have failed'),$fromHdrBugs);
	error_alert($err);
}
...
//if needed : if(!isset($datasetActiveFieldLabel))$datasetActiveFieldLabel=$datasetActiveField;
...
if(!$datasetActiveControl)$datasetActiveControl='toggleActiveObject(\'$dataset\', \'$GLOBALS[$datasetID]\', \'$datasetComponent\');';
if(!$datasetActiveInactiveImage)$datasetActiveInactiveImage='images/i/garbage2.gif';
if(!$datasetFieldList)$datasetFieldList='*';
if(!$modApType)$modApType='embedded';
if(!$modApHandle)$modApHandle='first';

//added 2012-12-16
if($profileSettings['dataobject']['columns'] && !$availableCols[$datasetGroup][$modApType][$modApHandle]['scheme']){
	$availableCols[$datasetGroup][$modApType][$modApHandle]['scheme']=$profileSettings['dataobject']['columns'];
}
...
require($MASTER_COMPONENT_ROOT.'/dataset_set_settings.php');
...
if($datasetOverrideSort){
	...
	foreach($availableCols[$datasetGroup][$modApType][$modApHandle]['scheme'] as $n=>$v){
		if($v['visibility']>=COL_AVAILABLE || !isset($v['visibility'])){
			if($datasetFieldList=='*')continue; //we do not sort unless we know projected fields for the table/view
			if($v['method']=='function' || (isset($v['sortable']) && $v['sortable']==false))continue; //cannot process this
			if(!in_array(strtolower($n), preg_split('/,\s*/',strtolower($datasetFieldList))))continue; //only sort by field
			//way to set DESC
			$existingSort[$n]=($v['orderDesc']==-1 ? -1 : 1);
			$_SESSION['userSettings']['default'.$dataset.'Sort']=$n;
			q("REPLACE INTO bais_settings SET UserName='$datasetUserName', vargroup='".$dataset."',varnode='default".$dataset."Sort',varkey='',varvalue='$n'");
			$_SESSION['userSettings']['default'.$dataset.'SortDirection']=$existingSort[$n];
...
if($submode!=='exportDataset' && !$datasetFocusViewCall){
	...
	hideInactive<?php echo $dataset?>=<?php echo ($hideInactive) ? $hideInactive : '0';?>;
	window.parent.hideInactive<?php echo $dataset?>=<?php echo $hideInactive ? 1 : 0;?>;
	</script><?php	
}
...
if(isset($filterOverride)){
...
}else if($submode=='updateDatasetFilters' || count($filters) || count($querytext)){
	...
	if(count($filterExpressions)){
		foreach($filterExpressions as $v){
			if(!trim($v))continue;
			if(!($x=parse_query(stripslashes($v),$datasetTable))){
				if($datasetQueryErrorHandling==QEHANDLE_CONTINUE){
					continue;
				}else{
					error_alert($err='Your query "' . stripslashes($v) . '" is not understood'.($parse_query['error']?': '.str_replace("'","\'",$parse_query['error']):''));
				}
			}
...
if($submode=='updateDatasetFilters' && $useStatusFilterOptions){
...
if(!$datasetFocusViewCall){
	...
	if($datasetQuery){
		...
		*/
		...
		$sql=$sql[0].' '.$orderBy . ' ' . ($submode!=='exportDataset'?$limitClause:'');
		if($datasetInternalFilter || $filterQuery || $datasetActive || ($useStatusFilterOptions && count($inStatusSet))){

            if(preg_match('/\bWHERE\b((.|\s)*?)(GROUP BY|HAVING|ORDER BY|LIMIT|$)/',$sql,$m)){
				$whereClause=trim($m[1]);
				$whereClause='1 AND ('.$whereClause.')';
				if($useStatusFilterOptions && count($inStatusSet))$whereClause.=" AND $statusFilterField IN('".implode("','",$inStatusSet)."')";
				if($datasetActiveUsage)$whereClause.=' AND ('.$datasetActive.')';
				if($datasetInternalFilter)$whereClause.=' AND ('.preg_replace('/^AND\s+/i','',trim($datasetInternalFilter)).')';
				if($filterQuery)$whereClause.=' '.$filterQuery; //has its own operators structure
				$sql=str_replace(trim($m[1]),$whereClause,$sql);
			}else{
				error_alert('Dataset query is allowed but must have a seed where clause');
				$sql=$datasetQuery;
			}
		}
	}else{
		$sql="SELECT $datasetFieldList FROM $datasetTable WHERE 1 ".
			...
			$filterQuery.
			" $orderBy ".($submode!=='exportDataset'?$limitClause:'');
	}
	unset($err,$err2);
	ob_start();
	$records=q($sql. ' limit 100', ($datasetArrayType ? $datasetArrayType : O_ARRAY), ERR_ECHO, $datasetCnx);
	$err=ob_get_contents();
	ob_end_clean();
	if($datasetDebug['time']==md5($MASTER_PASSWORD)){
		?><span class="gray" style="font-size:smaller;">Main query took: <?php echo $qr['time'];?> seconds</span><?php
	}
	if($err){
		echo $err;
		ob_start();
		$records=q(str_replace($orderBy,'',$sql), ($datasetArrayType ? $datasetArrayType : O_ARRAY_ASSOC), ERR_ECHO);
		$err2=ob_get_contents();
		ob_end_clean();
	}
	if($err || $err2){
		mail($developerEmail, 'Error file '.__FILE__.', line '.__LINE__,get_globals(),$fromHdrBugs);
	...
	//get count
	if($submode!=='exportDataset' && $limitClause){
		if($datasetQuery){
			$countSQL=preg_replace('/\bSELECT\b(.|\s)+?\bFROM\b/i','SELECT COUNT(*) FROM ',$sql);
		}else{
			$countSQL="SELECT COUNT(*) FROM $datasetTable WHERE 1 "	.
			($useStatusFilterOptions && count($inStatusSet) ? " AND $statusFilterField IN('".implode("','",$inStatusSet)."')":'').
			($datasetActiveUsage==true ? " AND $datasetActive" : '').
			($datasetInternalFilter ? ' AND ('.preg_replace('/^AND\s+/i','',trim($datasetInternalFilter)).')' : '').
			$filterQuery;
		}
		$countSQL=preg_replace('/\bLIMIT\s+[,0-9 ]+/i','',$countSQL);
		$count=q($countSQL, O_VALUE, ERR_ECHO);
...
/*
2012-07-16
* up to now $datasetQuery was not munging the WHERE clause, it now does with all additions to the where clause that are done using $datasetTable

2011-05-09
* added $datasetCnx: default= qx.defCnxMethod
2011-04-02 - 
* required default initial sort to have datasetFieldList != *, and also default field must not be sortable=false, or method=function
* exit with error if $dataset is not declared
* defined defalt values for modApType=embedded and modApHandle=first
