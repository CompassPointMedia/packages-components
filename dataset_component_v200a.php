...
/*
2013-01-16:
	* removed reference to $datasetID and replaced with function datasetID() - which gives field or value for compound primary keys as well
2012-12-18: version 2.00alpha
	* being used by systementry system, actually in console-rbrfm/components/comp_1000_systementry_dataobject_v100.php
	* here is where we start putting all the component settings into soft-coding, and develop the developer..
	* all of the previous notes have been moved below as this is going to be a radical slash-code change
	* key objectives are
		transition to jquery
		better object bindings so double-clicks e.g. mesh with *default* action i.e. open record, context menus match
		forms to configure the data object
			on setup
			and ongoing
		main items to configure:
			which columns show and in what way
				(this includes non-pure columns)
			colors and graphics/icons - tired of mickey mouse appearances
...
if(!function_exists('datasetID')){
	function datasetID($options=array()){
		/*created 2013-01-16 
		options:
			output=key|value(default)*/
		global $record, $recordPKField;
		extract($options);
		foreach($recordPKField as $v){
			$o[]=str_replace(':','::',$output=='key'? $v : $record[$v]);
		}
		return implode(':',$o);
	}
}
...
if(!isset($datasetTrim))$datasetTrim=true;
...
if(!$refreshComponentOnly && $submode!=='exportDataset'){
	...
	function mgeCol(e,n){
		...
		window.open('resources/bais_01_exe.php?mode=refreshComponent&component=<?php echo $datasetComponent . ($datasetFile ? ':'.$datasetFile.':'.md5($datasetFile.$MASTER_PASSWORD) : '')?>&_Profiles_ID_=<?php echo $_Profiles_ID_;?>&col='+n+'&visibility='+(posn> -1? 8 : 16),'w2');
	}
...
?><div id="<?php echo $datasetComponent?>" refreshparams="noparams">
	...
	echo '<h'.($datasetComponentHeadingLevel ? $datasetComponentHeadingLevel : '3').' id="'.$datasetComponent.'_heading">'?><?php echo $datasetWordPlural;?> <?php if(!$datasetHideCount){ ?>(<span id="<?php echo $datasetComponent?>_count"><?php echo $count;?></span>)<?php } ?> <?php 
	...
	//------ actual dataset ------
	?>
	<table border="0" cellspacing="0" cellpadding="0" class="<?php echo $datasetTableClass ? $datasetTableClass : 'dataset';?>" style="clear:both;">
			...
			<tr>
				...
				foreach($availableCols[$datasetGroup][$modApType][$modApHandle]['scheme'] as $handle=>$scheme){
					...
					if($submode=='exportDataset' && isset($datasetExportFields)){
						...
						if(!@in_array($handle,$datasetExportFields) || (isset($scheme['visibility']) && $scheme['visibility']<COL_AVAILABLE)){
							continue;
						}
					}else{
					...
					?>><?php 
						...
						if(($scheme['sortable'] || !isset($scheme['sortable'])) && !$datasetHideSort){ 
							...
							echo $datasetComponent . ($datasetFile ? ':'.$datasetFile.':'.md5($datasetFile.$MASTER_PASSWORD) : '');?>','<?php echo $handle?>',event, '<?php echo str_replace(' ','',$datasetSortParams);?>', '_Profiles_ID_=<?php echo $_Profiles_ID_;?>');"><?php
						}
						//output header
		...
		if($datasetFooterDisposition=='tabularControls'){
			...
			<td colspan="100%"><?php
			if($inBatching || $limitClause){
				$url='resources/bais_01_exe.php?mode=refreshComponent&_Profiles_ID_='.$_Profiles_ID_.'&component='.$datasetComponent . ($datasetFile ? ':'.$datasetFile.':'.md5($datasetFile.$MASTER_PASSWORD) : '').'&batch=';
				?><style type="text/css">
			...
			if(!$datasetHideFooterAddLink){
				?>&nbsp;&nbsp;<a href="<?php echo $datasetFocusPage?>?_Profiles_ID_=<?php echo $_Profiles_ID_;?>" onclick="return ow(this.href,'l1_<?php echo $datasetWord?strtolower(preg_replace('/[^a-z0-9]/i','',$datasetWord)):'object'.$_Profiles_ID_?>','750,700'<?php if($quasiResourceTypeField)echo ', true';?>);"><img src="/images/i/add_32x32.gif" width="32" height="32">Add<?php echo $datasetWord ? strtolower($datasetWord).' ':'';?>..</a><?php
			}
			?></td>
			</tr>
			</tfoot><?php
				...
				?><tr id="r_<?php echo datasetID()?>" onclick="<?php if($useHighlighting)echo $hNormalCoding;?>" ondblclick="<?php if($useHighlighting)echo $hNormalCoding;?>open<?php echo $dataset?>();" oncontextmenu="<?php if($useHighlighting)echo $hContextMenuCoding;?>" class="normal<?php echo fmod($i,2)?' alt':''?><?php echo $datasetAdditionalClass ? $datasetAdditionalClass : ''?>" deletable="<?php echo $deletable?>" <?php echo strtolower($datasetActiveFieldLabel ? $datasetActiveFieldLabel : $datasetActiveField);?>="<?php echo $$datasetActiveField?>"<?php if($datasetCustomAttributes)echo $datasetAttributes;?>>
					...
					if(!$datasetActiveHideControl){ 
						...
						if(!$$datasetActiveField){
							?><img src="/<?php echo $datasetActiveInactiveImage?>" align="absbottom" /><?php
						}else{
							?>&nbsp;<?php
						}
						?></td>
					...
					if(!$datasetHideEditControls){
						...
						if($datasetShowDeletion){
							if($deletable){
								?>&nbsp;&nbsp;<a title="Delete this <?php echo strtolower($datasetWord);?>" href="resources/bais_01_exe.php?mode=<?php echo $datasetDeleteMode ? $datasetDeleteMode : 'deleteObjects'?><?php echo $_Profiles_ID_ ? '&_Profiles_ID_='.$_Profiles_ID_:'';?>&<?php echo $datasetFocusQueryStringKey ? $datasetFocusQueryStringKey : 'Objects_ID';?>=<?php echo datasetID()?>" target="w2" onclick="if(!confirm('This will permanently delete this record.  Are you sure?'))return false;"><img src="/images/i/del2.gif" alt="delete" width="16" height="18" border="0" /></a><?php
							}else{
						...
						if($datasetFocusViewDeviceFunction){
							...
							if($datasetFocusViewDeviceFunction=='system_entry_focus'){
								mail($developerEmail, 'Error in '.$MASTER_USERNAME.':'.end(explode('/',__FILE__)).', line '.__LINE__,get_globals($notice='a reference to system_entry_focus needs changed in settings'),$fromHdrBugs);
							...
							if($dsgis){
								?><img src="/images/i/<?php echo $focusViewEditImage ? $focusViewEditImage : 'edit2.gif'?>" <?php echo $dsgis[2]?> alt="edit" /><?php
							}else{
								?>[edit]<?php
							}
							?></a> &nbsp;<?php
					...
					foreach($availableCols[$datasetGroup][$modApType][$modApHandle]['scheme'] as $handle=>$scheme){
						...
						if(is_array($a=$scheme['form_field'])){
							$out='';
							if(!$recordHeader){
								$recordHeader=true;
								ob_start();
								if($recordPKField){
									foreach($recordPKField as $v){
										?><input type="hidden" name="data[<?php echo $v;?>][]" class="_primary_" value="<?php echo $record[$v];?>" /><?php
										echo "\n";
									}
									?><input type="hidden" name="data[_change_][]" class="_change_" /><?php
									echo "\n";
								}else{
									//what?
								}
							...
							if(!$a['type']){
								//we need to figure out the best type if input element
								$a['type']='text';
							}
							...
							switch($a['type']){
								case 'text':
								case 'password':
									if(!$a['attributes']['size'])$a['attributes']['size']=10;
									?><input type="<?php echo $a['type'];?>" name="data[<?php echo $handle;?>][]" class="dsfield<?php if($n=$a['attributes']['class'])echo ' '.$n;?>" value="<?php echo h($record[$handle]);?>" <?php if($a['attributes'])foreach($a['attributes'] as $n=>$v)if($n!=='class')echo ' '.$n.'="'.h($v).'"';?> /><?php
								break;
								case 'textarea':
								break;
							}
							...
							if($echoed && !$echonotified){
								$echonotified=true;
								mail($developerEmail, 'Error file '.__FILE__.', line '.__LINE__,'print: '.$echoed. "\n\n". get_globals(),$fromHdrBugs);
							}
						}else{
							$out=$record[$handle];
						...
						if($submode=='exportDataset' && $datasetExportMethod>1){
							...
							if(($n=$scheme['exportKey']) && $datasetExportMethod>2){
								if(is_array($n)){
									...
									$rowOutput.=$quote . 
									str_replace($datasetExportWrapper, $datasetExportWrapperEscape, $datasetTrim ? trim($record[$n]) : $record[$n]). 
									$quote.$datasetExportSeparator;
								}
							}
							$quote=(preg_match('/['.$datasetExportSeparator.$datasetExportWrapper.']/', $datasetTrim ? trim($out) : $out) ? $datasetExportWrapper : '');
							$rowOutput.=$quote . str_replace($datasetExportWrapper, $datasetExportWrapperEscape, $datasetTrim ? trim($out) : $out). $quote.$datasetExportSeparator;
						}else{
...
if($refreshComponentOnly && $mode!=='refreshComponent' /* ironically so, because in this case it's done externally */){
	?><script language="javascript" type="text/javascript">
	...
	<?php if(false && $updateDatasetFilters){ ?>
