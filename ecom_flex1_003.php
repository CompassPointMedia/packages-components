...
if($useSKUAsImage){
	...
	$dims='';
	unset($get_image);
	$src=get_image($rdp['SKU'], $$prodImgMainArray, $prodImgMainOptions);
	...
	<span class="prodImgMain"><img id="prodSlide_<?php echo $flexIdx;?>_0" src="<?php echo $prodMainImgPath . $src;?>" <?php echo $dims?> alt="<?php echo $prodImgAlt ?>" style="display:inline;" /><?php
	if(count($prodSlides)){
		foreach($prodSlides as $n=>$v){
			?><img id="prodSlide_<?php echo $flexIdx?>_<?php echo $n+1;?>" title="<?php echo is_numeric($v['slidename'])?'Picture '.$v['slidename']:h($v['slidename'])?>" src="<?php  echo $prodMainImgPath . $v['name'];?>" <?php echo $v['dims']?> style="display:none;" /><?php
		}
	}
	?></span><?php
...
if((count($b=q("SELECT *
	...
	ORDER BY IF(UnitPrice!=0,1,2), UnitPrice", O_ARRAY)) && $showRelatedItems) || $adminMode){
	ob_start();
