...
if(!$suppressWWWRedirection && substr($_SERVER['SERVER_NAME'],0,3)!=='www'){
	header('Location: http'.($secureProtocolPresent?'s':'').'://www.'.$_SERVER['SERVER_NAME'].$_SERVER['PHP_SELF'].($_SERVER['QUERY_STRING'] ? '?'.$_SERVER['QUERY_STRING'] : ''));
	exit;
}
...
if($_SERVER['REDIRECT_STATUS']==404){
    ...
    if(!strlen($thispage) || !isset($thisfolder)){
        $a=preg_split('/\\\|\//',$_SERVER['SCRIPT_NAME']);
        $thispage=$a[count($a)-1];
        if(count($a)>2){
            $thisfolder=$a[count($a)-2];
        }else{
...
$businessTitles=array('Owner','President/CEO','CIO','CFO','Marketing Director','Sales Manager','Sales Representative','Administrator','Other');
...
if(!$suppressFunctionFiles){
	...
	require($FUNCTION_ROOT.'/function_CMSB_v300.php');
	...
	require($FUNCTION_ROOT.'/function_array_subkey_sort_v201.php');
...
//implement maillog by default
if(!isset($enhanced_mail['logmail']))$enhanced_mail['logmail']=true;
...
eval('function stripslashes_deep($value){
