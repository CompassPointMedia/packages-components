...
if(!isset($datasetDefaultBatch))$datasetDefaultBatch=($globalBatchThreshold ? $globalBatchThreshold : 5);
...
if(isset($filterOverride)){
		...
		foreach($filterExpressions as $v){
			...
			if(!($x=parse_query(stripslashes($v),$datasetTable))){
				if($datasetQueryErrorHandling==QEHANDLE_CONTINUE){
					...
					error_alert($err='Your query "' . str_replace("'","\\\'",$v) . '" is not understood'.($parse_query['error']?': '.str_replace("'","\\\'",$parse_query['error']):''));
				}
			}
	...
	//2013-04-02: we also reset batching any time filter Queries have been reset
	unset($limitClause,$limit,$batch,$_SESSION['userSettings']['default'.$dataset.'Batch']);
	$limit='1,'.$datasetDefaultBatch;
...
if($testLimit)$limitClause='LIMIT '.$testLimit;
//run the query
if(!$datasetFocusViewCall){
		...
		*/
		...
		if($datasetInternalFilter || $filterQuery || $datasetActive || ($useStatusFilterOptions && count($inStatusSet))){
			if(preg_match('/\bWHERE\b((.|\s)*?)(GROUP BY|HAVING|ORDER BY|LIMIT)/',$sql,$m)){
				...
				if($datasetInternalFilter)$whereClause.=' AND ('.$datasetInternalFilter.')';
	...
	ob_start();
	$records=q($sql, ($datasetArrayType ? $datasetArrayType : O_ARRAY_ASSOC), ERR_ECHO, $datasetCnx);
	$err=ob_get_contents();
	ob_end_clean();
	...
	if($submode!=='exportDataset' && $limitClause){
		...
		$countSQL=preg_replace('/\s+LIMIT\s+[ ,0-9]+$/i','',trim($countSQL));
